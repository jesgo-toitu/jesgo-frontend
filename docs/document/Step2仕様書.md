...
<tr><td>PR</td><td>2022-10-13 山本(P4mohnet)</td></tr>
</table>
...
以下はオプショナルとなるが実装について要求されうる仕様であり検討を要する。

- HISとの連携
  - HISのログイン情報に基づきJESGOのログインをバイパスする
  - HISの患者IDで患者リスト画面を経由せずに患者を開く
...
## 8. HISとの連携
ユーザと患者IDをURLで指定して連携が可能になるように実装する。

セキュリティの面で、通常アクセスとは以下の点を区別する
- リスト画面には戻らず、戻るではウインドウを閉じる
- ユーザ管理、システム管理は機能しない
- 存在しないユーザからのアクセスは、拒絶もしくは閲覧ユーザとして機能をシステム設定で選択する
