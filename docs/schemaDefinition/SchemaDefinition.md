...
|37 | サブスキーマ：横方向に入力項目を展開 | 拡張 |  | inline | 完了 | ○ |  | Ver1.0.3: array以外のobjectもinlineに対応 |
...
|42 |  | 拡張 |  | recurrence | 未 | × |  | |
|43 | ~~その他：DBから値の取得~~ | ~~拡張~~ | ~~jesgo:get~~ | ~~eventdate~~ | 実装対象外 | × |  | 2023/02/02：jesgo:getは対応しない。計算値が欲しい場合はプラグインで代用する |
|44 |  | ~~拡張~~ |  | ~~age~~ | 実装対象外 | × |  | |
|45 |  | ~~拡張~~ |  | ~~month~~ | 実装対象外 | × |  | |
|46 |  | ~~拡張~~ |  | ~~week~~ | 実装対象外 | × |  | |
|47 |  | ~~拡張~~ |  | ~~day~~ | 実装対象外 | × |  | |
|48 | ドキュメントメニュー | 拡張 | jesgo:copy | boolean | 完了 | × |  | Ver1.0.3にて対応済み |
