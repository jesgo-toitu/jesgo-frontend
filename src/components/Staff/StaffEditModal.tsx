import React, { MouseEventHandler, ReactEventHandler, SFC, useCallback, useEffect, useState } from "react";
import {Modal, Button, ControlLabel, FormControl, FormGroup, ModalProps} from 'react-bootstrap';
import apiAccess, { METHOD_TYPE, RESULT } from "../../common/ApiAccess";
import { DISPLAYNAME_MAX_LENGTH, loginIdCheck, passwordCheck, rollList, StaffErrorMessage } from "../../common/StaffMaster";
...
export const StaffEditModalDialog = (
    props :{
    onHide:Function;
    onOk:Function;
  ...
  const addUser = async () => {
      ...
      let checkDisplayName = displayName.trim();
      if( !checkDisplayName){
      ...
      var checkPasswordConfilm = passwordConfilm.trim();
      if(!checkPassword){
      ...
      const returnApiObject = await apiAccess(
        METHOD_TYPE.POST,
        `signup/`,
        { name: loginId, display_name: displayName, password: password, roll_id: roll}
      );
      console.log(returnApiObject);
      if (returnApiObject.statusNum === RESULT.NORMAL_TERMINATION) {
        alert("登録しました");
        props.onOk();
      } else if( returnApiObject.statusNum === RESULT.FAILED_USER_ALREADY_REGISTERED ) {
  ...
  const updateUser = async () => {
    ...
    const returnApiObject = await apiAccess(
      METHOD_TYPE.POST,
      `editUser/`,
      { user_id: userId,  name: loginId, display_name: displayName, password: password, roll_id: roll}
    );
    console.log(returnApiObject);
    if (returnApiObject.statusNum === RESULT.NORMAL_TERMINATION) {
      alert("更新しました");
